...
public interface ContaClienteRepository extends JpaRepository<ContaCliente, Integer> {
	@Query(value = "select * from XPTO.CONTA_CLIENTE c where c.ID_CLIENTE = :idCliente", nativeQuery = true)
	List<ContaCliente> findByIdCliente(@Param("idCliente") Integer idCliente);

	@Modifying
	@Transactional
	@Query(value = "delete XPTO.CONTA_CLIENTE c where c.ID_CLIENTE = :idCliente and c.ID_CONTA_BANCARIA = :idContaBancaria", nativeQuery = true)
